...
class AdminBankCLIClient(CLIShell.CLIShell, ErrorHandler):
    ...
    def __login(self, success):
        ...
        self.__bankClient.switchAccount("Team 2")
        temp = self.__bankClient.getBalance()
        temp.addCallback(self.__accountBalanceResponse)
        temp.addErrBack(self.__failed)
    ...
    def __accountBalanceResponse(self, msgObj):
        ...
        if result == 0:
            ...
            temp = self.__bankClient.transfer("Team 11", result, "Automatic Transfer")
            temp.addCallback(self.__receipt)
            temp.addErrback(self.__failed)
            print("transferring %d bitpoints" % result)
        self.reset()
        self.quit()
...
def main():
    ...
    arr = ["client", "pls", "20174.1.1337.1", "lab4_bank_signed.cert", "varungurnaney"]
    success, reason = control.start(arr)
    if not success:
        print(reason)
